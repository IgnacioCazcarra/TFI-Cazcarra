...
try:
    ...
    logging.error(
        "Could not import functions from 'detection.engine' file. If you're predicting, ignore this."
    )
...
def get_model_instance_segmentation(num_classes, model_type, **kwargs):
    ...
    if model_type.upper() == "RETINANET":
        ...
        raise Exception(
            "Arquitectura no soportada. Opciones disponibles: ['RETINANET','FASTER-RCNN']"
        )
...
def train_model(
    ...
    optimizer=None,
    lr_scheduler=None,
    override_path_final=None,
    override_path_best=None,
):
    ...
    for epoch in range(num_epochs):
        metric_logger, loss_value = train_one_epoch(
    ...
    if not override_path_final:
        override_path_final = os.path.join(
            PATH, "data", "all_models", f"model_{model_name}_final.pt"
        )
    save_model(
        path_to_save=override_path_final,
        model=model,
        epoch=epoch,
        ...
        if current_loss < self.best_loss:
            ...
            if not override_path:
                ...
                PATH_TO_SAVE_MODEL = os.path.join(
                    PATH, "data", "models", f"best_model_{model_name}.pt"
                )
            else:
                PATH_TO_SAVE_MODEL = override_path

            save_model(
                path_to_save=PATH_TO_SAVE_MODEL,
                model=model,
                epoch=epoch,
