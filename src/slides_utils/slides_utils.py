...
def default_ibs(img_src, ann_src, img_dst, ann_dst):
    slicer = ibs.Slicer()
    slicer.config_dirs(
        img_src=img_src, ann_src=ann_src, img_dst=img_dst, ann_dst=ann_dst
    )
...
def __slice_images(slicer, tile_size, tile_overlap, number_tiles):
    ...
    for file in sorted(glob.glob(slicer.im_src + os.sep + "*")):
        ...
        if number_tiles > 0:
            ...
            tile_w, tile_h = int(math.floor(im.size[0] / n_cols)), int(
                math.floor(im.size[1] / n_rows)
            )
...
def slice_img_from_prediction(img, tile_size, tile_overlap, number_tiles):
    if number_tiles > 0:
        n_cols, n_rows = calc_columns_rows(number_tiles)
        tile_w, tile_h = int(math.floor(img.size[0] / n_cols)), int(
            math.floor(img.size[1] / n_rows)
        )
        tile_size = (tile_w, tile_h)
...
def validate_tile_size(tile_size, img_size=None):
    ...
    if img_size is None:
        if isinstance(tile_size, tuple):
            if len(tile_size) != 2:
                raise ValueError(
                    "Tile size must be a tuple of size 2 i.e., (w, h). The tuple provided was {}".format(
                        tile_size
                    )
                )
        else:
            raise TypeError(
                "Tile size must be a tuple. The argument was of type {}".format(
                    type(tile_size)
                )
            )
    else:
        if isinstance(img_size, tuple):
            if (
                (sum(tile_size) >= sum(img_size))
                or (tile_size[0] > img_size[0])
                or (tile_size[1] > img_size[1])
            ):
                raise ValueError(
                    "Tile size cannot exceed image size. Tile size was {} while image size was {}".format(
                        tile_size, img_size
                    )
                )
        else:
            raise TypeError(
                "Image size must be a tuple. The argument was of type {}".format(
                    type(img_size)
                )
            )
...
def __slice_bboxes(slicer, tile_size, tile_overlap, number_tiles):
    ...
    for xml_file in sorted(glob.glob(slicer.ann_src + os.sep + "*.xml")):
        ...
        im_w, im_h = int(root.find("size")[0].text), int(root.find("size")[1].text)
        ...
        for tile in tiles:
            ...
            voc_writer = Writer(
                "{}{}{}{}".format(slicer.ann_dst, os.sep, img_no_str, extn),
                tile_w,
                tile_h,
            )
            for obj in objects:
                ...
                if points_info == Points.NONE:
                    ...
                    not slicer.keep_partial_labels
                    ...
                    new_lbl = (0, obj_lbl[1] - tile[1], obj_lbl[2] - tile[0], tile_h)

                elif points_info == Points.P3:
                    new_lbl = (obj_lbl[0] - tile[0], 0, tile_w, obj_lbl[3] - tile[1])

                elif points_info == Points.P4:
                    new_lbl = (0, 0, obj_lbl[2] - tile[0], obj_lbl[3] - tile[1])
            ...
            if slicer.ignore_empty_tiles and (empty_count == len(objects)):
                ...
                voc_writer.save("{}{}{}.xml".format(slicer.ann_dst, os.sep, img_no_str))
...
def unify_images(img, boxes_per_tile):
    ...
    for tile, prediction in boxes_per_tile.items():
        coords_to_add = torch.Tensor(
            list(map(lambda i, j: i - j, literal_eval(tile), literal_eval(first_tile)))
        )
    ...
    return {
        "boxes": torch.from_numpy(all_boxes.reshape((-1, 4))),
        "scores": torch.from_numpy(all_scores),
        "labels": torch.from_numpy(all_labels),
    }


def predict_tiles(img, model, is_yolo, transform, min_size=600, max_size=1333):
    tiles = slice_img_from_prediction(
        img, tile_size=None, tile_overlap=0.0, number_tiles=6
    )
    preds_image = {}
    with torch.no_grad():
        for tile in tiles:
