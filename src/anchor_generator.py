    ...
    def generate_anchors(
        ...
        for i in range(len(base_anchors)):
            for j in range(len(scales)):
                if i == 0 and j==0:
                    all_anchors = base_anchors[i]*scales[j]
                else:
                    all_anchors = torch.cat((all_anchors, base_anchors[i]*scales[j]))
        return all_anchors.round()

    def set_cell_anchors(self, dtype: torch.dtype, device: torch.device):
        self.cell_anchors = [cell_anchor.to(dtype=dtype, device=device) for cell_anchor in self.cell_anchors]

    def num_anchors_per_location(self) -> List[int]:
        return [len(s) * len(a) * len(sc) for s, a, sc in zip(self.sizes, self.aspect_ratios, self.scales)]
