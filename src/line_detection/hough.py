...
def create_line(new_line, without_current, dst_threhold=20):
    ...
    while not flag:
        ...
        if dst <= dst_threhold:
            ...
            new_line, to_delete2 = add_from_end(
                current_line=new_line,
                without_current=without_current,
...
def hough_detecting(all_points, **kwargs):
    ...
    while not main_flag:
        ...
        new_line, to_delete = create_line(
            new_line=new_line, without_current=without_current, **kwargs
        )
...
def unify_cardinalidades(
    img, lines, cardinalidades, plot=False, path_to_save_conexiones=None
):
    if not lines:
        return {}
    dict_cardinalidades = {}
    ...
    if plot and path_to_save_conexiones:
        plot_results(img, new_dict_cardinalidades, dict_lines).save(
            path_to_save_conexiones
        )
    return new_dict_cardinalidades
...
def clean_cardinalidades(cardinalidades, tablas, distance_threshold=50):
    ...
    return [
        idx
        for idx, c in enumerate(cardinalidades)
        if nearest_tabla_from_cardinalidad(
            c, tablas, distance_threshold=distance_threshold
        )
    ]
...
def find_lines(tablas, cardinalidades, img, offset_tablas=5, **kwargs):
    offset = np.array(
        [-offset_tablas, -offset_tablas, offset_tablas, offset_tablas]
    ).reshape(1, 4)
...
def nearest_tabla_from_cardinalidad(cardinalidad, tablas, distance_threshold=999999):
    cardinalidad = (
        literal_eval(cardinalidad) if isinstance(cardinalidad, str) else cardinalidad
    )
...
def sep_line(line, tablas, cardinalidades_labels_dict):
    ...
    try:
        ...
        logging.warning(
            f"Error al separar tablas! {e}. Chequear las bounding boxes pasadas. Salteando.."
        )
    finally:
...
def get_pairs(
    boxes_tablas,
    boxes_cardinalidades,
    labels_cardinalidades,
    img,
    ...
    **kwargs,
    ...
    valid_cardinalidades = clean_cardinalidades(
        cardinalidades, tablas, distance_threshold=distance_threshold
    )
    ...
    cardinalidades_labels_dict = {
        str(k.tolist()): v for k, v in zip(cardinalidades, cardinalidades_labels)
    }

    for line_name, line in find_lines(
        img=img, tablas=tablas, cardinalidades=cardinalidades, **kwargs
    ).items():
        (tabla_a, tabla_b), (card_a_label, card_b_label) = sep_line(
            line, tablas, cardinalidades_labels_dict=cardinalidades_labels_dict
        )
