...
def filter_predictions(
    predictions, score_threshold=0.5, nms_threshold=0.5, return_labels=False
):
    ...
    if score:
        try:
            ...
            img = cv2.putText(
                img,
                str(round(score, 2)),
                (int(xmax) - 1, int(ymin) - 1),
                cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=min(w, h) * FONT_SCALE,
                color=color,
            ...
            logging.warn(e)
            logging.warn(
                "No se pudo graficar el puntaje de la predicción debido a su posición. Salteando.."
            )
    return cv2.rectangle(
        img, (int(xmin), int(ymin)), (int(xmax), int(ymax)), color, thickness
    )
...
def _validate_model_options(
    object_to_predict, object_types=["tablas", "cardinalidades"]
):
